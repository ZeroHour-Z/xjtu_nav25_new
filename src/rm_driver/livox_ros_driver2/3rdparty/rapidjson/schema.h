...
public:
    ...
    virtual void
    AddDependencySchemaError(const SValue& souceName, ISchemaValidator* subvalidator) = 0;
    ...
    RAPIDJSON_FORCEINLINE bool EndValue(Context& context) const {
        ...
        if (oneOf_.schemas) {
            ...
            for (SizeType i = oneOf_.begin; i < oneOf_.begin + oneOf_.count; i++)
                if (context.validators[i]->IsValid()) {
                    if (oneValid) {
                        context.error_handler.NotOneOf(
                            &context.validators[oneOf_.begin],
                            oneOf_.count
                        );
        ...
        if (const ValueType* v = GetMember(value, name)) {
            if (v->IsArray() && v->Size() > 0) {
                ...
                for (SizeType i = 0; i < out.count; i++)
                    schemaDocument
                        .CreateSchema(&out.schemas[i], q.Append(i, allocator_), (*v)[i], document);
        ...
        if (v.GetType() == kObjectType) {
            ...
            for (typename ValueType::ConstMemberIterator itr = v.MemberBegin();
                ...
                CreateSchemaRecursive(
                    0,
                    pointer.Append(itr->name, allocator_),
                    itr->value,
                    document
                );
...
public:
  ...
  */
    ...
    void DisallowedItem(SizeType index) {
        currentError_.SetObject();
        currentError_
            .AddMember(GetDisallowedString(), ValueType(index).Move(), GetStateAllocator());
        AddCurrentError(SchemaType::GetAdditionalItemsString(), true);
    }
    void TooFewItems(SizeType actualCount, SizeType expectedCount) {
        AddNumberError(
            SchemaType::GetMinItemsString(),
    ...
    void TooManyProperties(SizeType actualCount, SizeType expectedCount) {
        AddNumberError(
            SchemaType::GetMaxPropertiesString(),
            ValueType(actualCount).Move(),
            SValue(expectedCount).Move()
        );
    }
    void TooFewProperties(SizeType actualCount, SizeType expectedCount) {
        AddNumberError(
            SchemaType::GetMinPropertiesString(),
            ValueType(actualCount).Move(),
            SValue(expectedCount).Move()
        );
    }
...
private:
    ...
    void AppendToken(const Ch* str, SizeType len) {
        documentStack_.template Reserve<Ch>(
            1 + len * 2
        ); // worst case all characters are escaped as two characters
    ...
    void AddErrorLocation(ValueType& result, bool parent) {
        ...
        sb.Clear();
        memcpy(
            sb.Push(CurrentSchema().GetURI().GetStringLength()),
    ...
    internal::Stack<StateAllocator> schemaStack_; //!< stack to store the current path of schema
        //!< (BaseSchemaType *)
