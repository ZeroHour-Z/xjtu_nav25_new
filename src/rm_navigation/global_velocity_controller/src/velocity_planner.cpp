...
void VelocityPlanner::setConfig(const VelocityPlannerConfig& config) {
    config_ = config;
}
...
double VelocityPlanner::computeDynamicLookahead(std::size_t start_index) const {
    ...
    const double lookahead = config_.max_lookahead_distance
...
bool VelocityPlanner::computeTarget(
    double current_x_m,
    double current_y_m,
    double current_yaw,
    double& target_vx_m,
    ...
    if (dist < 1e-6) {
        ...
        const double local_speed = std::max(
            config_.min_target_speed,
            config_.target_speed * slow_factor * lookahead_ratio
        );
