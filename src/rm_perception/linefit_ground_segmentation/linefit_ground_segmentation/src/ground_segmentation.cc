...
void GroundSegmentation::getLines(std::list<PointLine>* lines) {
    ...
    for (i = 0; i < params_.n_threads; ++i) {
        ...
        thread_vec[i] = std::thread(
            &GroundSegmentation::lineFitThread,
            this,
            start_index,
            end_index,
            lines,
...
void GroundSegmentation::lineFitThread(
    ...
    for (unsigned int i = start_index; i < end_index; ++i) {
        ...
        if (visualize) {
            ...
            for (auto line_iter = segment_lines.begin(); line_iter != segment_lines.end();
                 ++line_iter) {
...
pcl::PointXYZ
GroundSegmentation::minZPointTo3d(const Bin::MinZPoint& min_z_point, const double& angle) {
    pcl::PointXYZ point;
...
void GroundSegmentation::assignCluster(std::vector<int>* segmentation) {
    ...
    for (unsigned int i = 0; i < params_.n_threads; ++i) {
        ...
        thread_vec[i] = std::thread(
            &GroundSegmentation::assignClusterThread,
            this,
            start_index,
            end_index,
            segmentation
...
void GroundSegmentation::assignClusterThread(
    const unsigned int& start_index,
    const unsigned int& end_index,
...
void GroundSegmentation::insertionThread(
