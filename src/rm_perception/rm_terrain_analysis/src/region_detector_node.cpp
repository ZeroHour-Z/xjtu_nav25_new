...
private:
    ...
    // 判断点是否在多边形内 (射线法)
    bool
    isPointInPolygon(double x, double y, const std::vector<std::pair<double, double>>& polygon) {
        if (polygon.size() < 3)
            return false;
    ...
    // 计算点到多边形边界的最近距离
    double
    distanceToPolygon(double x, double y, const std::vector<std::pair<double, double>>& polygon) {
        if (polygon.size() < 3)
