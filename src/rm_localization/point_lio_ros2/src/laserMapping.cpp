...
void publish_frame_world(
    const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& pubLaserCloudFullRes,
    const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& pubLaserCloudFullResDense
) {
    if (scan_pub_en) {
        // Publish downsampled map/frame (existing behavior)
        sensor_msgs::msg::PointCloud2 laserCloudmsg;
        ...
        //   size_t full_size = feats_undistort->points.size();
        //   PointCloudXYZI::Ptr laserCloudFullWorld(new PointCloudXYZI(full_size, 1));
        //   for (size_t i = 0; i < full_size; ++i) {
        //     pointBodyToWorld(&feats_undistort->points[i], &laserCloudFullWorld->points[i]);
        //   }
        //   sensor_msgs::msg::PointCloud2 laserDenseMsg;
        //   pcl::toROSMsg(*laserCloudFullWorld, laserDenseMsg);
        //   laserDenseMsg.header.stamp = get_ros_time(lidar_end_time);
        //   laserDenseMsg.header.frame_id = "camera_init";
        //   pubLaserCloudFullResDense->publish(laserDenseMsg);
        // }
...
int main(int argc, char** argv) {
    ...
    downSizeFilterSurf
        .setLeafSize(filter_size_surf_min, filter_size_surf_min, filter_size_surf_min);
    ...
    kf_output
        .init_dyn_share_modified_3h(get_f_output, df_dx_output, h_model_output, h_model_IMU_output);
    ...
    auto pub_laser_cloud_full_res_dense =
        nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_registered_dense", 1000);
        ...
        if (sync_packages(Measures)) {
            ...
            if (!p_imu->after_imu_init_) // !p_imu->UseLIInit &&
            {
                if (!p_imu->imu_need_init_) {
                    ...
                    std::cout << "rot_init" << rot_init << std::endl;
            ...
            if (scan_pub_en || pcd_save_en)
                publish_frame_world(pub_laser_cloud_full_res, pub_laser_cloud_full_res_dense);
            if (scan_pub_en && scan_body_pub_en)
                publish_frame_body(pub_laser_cloud_full_res_body);

            if (init_map && Measures.lidar_last_time > 0.0) {
                std::lock_guard<std::mutex> lock(GetOrientationEkfMutex());
                ImuOrientationEkf& orientation_ekf = GetOrientationEkf();
                Eigen::Quaterniond imu_before = orientation_ekf.quaternion();
                Eigen::Quaterniond lidar_orientation = use_imu_as_input
                    ? Eigen::Quaterniond(kf_input.x_.rot)
                    : Eigen::Quaterniond(kf_output.x_.rot);
                if (orientation_ekf.alignWithLidar(Measures.lidar_last_time, lidar_orientation)) {
                    Eigen::Quaterniond imu_after = orientation_ekf.quaternion();
                    Eigen::Quaterniond delta = imu_before.conjugate() * imu_after;
                    Eigen::Vector3d delta_rpy =
                        delta.toRotationMatrix().eulerAngles(0, 1, 2) * kDegreesPerRad;
                    double angular_correction_deg =
                        delta.angularDistance(Eigen::Quaterniond::Identity()) * kDegreesPerRad;
                    double time_gap =
                        std::abs(orientation_ekf.lastTimestamp() - Measures.lidar_last_time);
            ...
            if (runtime_pos_log) {
                ...
                {
                    aver_time_icp =
                        aver_time_icp * (frame_num - 1) / frame_num + update_time / frame_num;
                }
                aver_time_match =
                ...
                aver_time_propag =
                    aver_time_propag * (frame_num - 1) / frame_num + propag_time / frame_num;
                ...
                printf(
                    ...
                    "%0.6f  ave ICP: %0.6f  map incre: %0.6f ave total: %0.6f icp: %0.6f propogate: "
                    "%0.6f \n",
                    t1 - t0,
                    aver_time_match,
                    aver_time_solve,
                ...
                if (!publish_odometry_without_downsample) {
                    if (!use_imu_as_input) {
                        ...
                        fout_out << setw(20) << Measures.lidar_beg_time - first_lidar_time << " "
                                 ...
                                 << " " << kf_input.x_.vel.transpose() << " "
                                 << kf_input.x_.bg.transpose() << " " << kf_input.x_.ba.transpose()
                                 << " " << kf_input.x_.gravity.transpose() << " "
                                 << feats_undistort->points.size() << '\n';
                    }
                }
    ...
    printf(
        "start saving map to pcd file, total points: %d\n file path: %s",
        (int)pcl_wait_save->size(),
