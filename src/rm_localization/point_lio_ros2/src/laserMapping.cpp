...
int main(int argc, char** argv) {
  ...
  kf_output.init_dyn_share_modified_3h(get_f_output, df_dx_output, h_model_output,
                                       h_model_IMU_output);
    ...
    if (sync_packages(Measures)) {
      ...
      if (init_map && Measures.lidar_last_time > 0.0) {
        ...
        if (orientation_ekf.alignWithLidar(Measures.lidar_last_time, lidar_orientation)) {
          ...
          double time_gap = std::abs(orientation_ekf.lastTimestamp() - Measures.lidar_last_time);
          ...
          if (alignment_debug_counter % 5 == 0 || angular_correction_deg > 0.1) {
            RCLCPP_INFO(LOGGER,
                        ...
                        "delta_rpy=[%.2f %.2f %.2f]",
                        alignment_debug_counter, Measures.lidar_last_time, time_gap,
                        angular_correction_deg, delta_rpy(0), delta_rpy(1), delta_rpy(2));
            Eigen::Vector3d imu_after_rpy =
                imu_after.toRotationMatrix().eulerAngles(0, 1, 2) * kDegreesPerRad;
            RCLCPP_INFO(LOGGER, "imu_after_rpy: [%.3f, %.3f, %.3f]", imu_after_rpy(0),
                        imu_after_rpy(1), imu_after_rpy(2));
          }
        }
      }
  ...
  printf("start saving map to pcd file, total points: %d\n file path: %s",
         (int) pcl_wait_save->size(), pcd_save_file_path.c_str());
