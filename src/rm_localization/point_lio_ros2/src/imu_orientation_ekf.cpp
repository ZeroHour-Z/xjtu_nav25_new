...
ImuOrientationEkf::ImuOrientationEkf()
    : state_(Eigen::Matrix<double, 6, 1>::Zero()),
      ...
      angular_velocity_process_noise_(Eigen::Matrix3d::Identity() * kDefaultAngularVelocityProcessVariance),
      gyro_measurement_noise_(Eigen::Matrix3d::Identity() * kDefaultGyroMeasurementVariance),
      orientation_measurement_noise_(Eigen::Matrix3d::Identity() * kDefaultOrientationMeasurementVariance),
      max_history_duration_(kDefaultHistoryDurationSeconds),
      last_timestamp_(0.0),
      initialized_(false) {}

void ImuOrientationEkf::configure(double orientation_process_variance,
    ...
    angular_velocity_process_noise_ = Eigen::Matrix3d::Identity() * angular_velocity_process_variance;
...
void ImuOrientationEkf::reset(const Eigen::Quaterniond& orientation,
...
void ImuOrientationEkf::updateWithOrientation(double timestamp, const Eigen::Quaterniond& orientation) {
    if (!initialized_) {
...
bool ImuOrientationEkf::alignWithLidar(double lidar_timestamp, const Eigen::Quaterniond& lidar_orientation) {
    if (!initialized_) {
    ...
    if (insert_pos != history_.end() && std::abs(insert_pos->stamp - lidar_timestamp) <= kTimeEqualityEps) {
...
void ImuOrientationEkf::applyOrientationMeasurement(const Eigen::Vector3d& euler, double timestamp) {
    Eigen::Matrix<double, 3, 6> H = Eigen::Matrix<double, 3, 6>::Zero();
