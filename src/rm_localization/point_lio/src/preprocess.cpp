...
void Preprocess::process_cut_frame_livox(
    ...
    for (uint i = 1; i < plsize; i++) {
            ...
            && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
        {
            ...
            if (valid_point_num % point_filter_num == 0) {
                ...
                double dist = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        ...
        pl_surf[i].curvature +=
            rclcpp::Time(msg->header.stamp).seconds() * 1000 - last_frame_end_time;
...
void Preprocess::process_cut_frame_pcl2(
    ...
    if (lidar_type == VELO16) {
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            if (dist < blind * blind || dist > det_range * det_range || isnan(added_pt.x)
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature =
                (pl_orig.points[i].timestamp - rclcpp::Time(msg->header.stamp).seconds())
                * 1000; //s to ms
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::SharedPtr& msg) {
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        double range = pl_orig.points[i].x * pl_orig.points[i].x
            + pl_orig.points[i].y * pl_orig.points[i].y + pl_orig.points[i].z * pl_orig.points[i].z;
