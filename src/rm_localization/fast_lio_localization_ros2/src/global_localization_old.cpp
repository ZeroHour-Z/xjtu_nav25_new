...
private:
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cropGlobalMapInFOV(
        ...
        if (!submap->empty()) {
            auto ds =
                pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>(*submap));
            if (ds->size() > 200000)
    ...
    void cbInitGlobalMap(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        RCLCPP_INFO(
            this->get_logger(),
            "Global map received. points=%zu (downsampled)",
